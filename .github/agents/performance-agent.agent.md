...
argument-hint: "e.g. Find why the task list is slow"
handoffs:
  - label: Add guardrail tests
    agent: test-writer
    prompt: Add tests or benchmarks that guard against the performance regression we fixed.
  - label: Review observability
    agent: observability-engineer
    prompt: Add or review metrics/traces needed to keep this issue visible.
