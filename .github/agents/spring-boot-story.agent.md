...
handoffs:
  - label: Add tests
    agent: test-writer
    prompt: Add focused backend tests for the changes from the last story.
  - label: Review API
    agent: pr-reviewer
    prompt: Review the backend diff for API, security, and operability gaps.
  - label: Tune data layer
    agent: database-engineer
    prompt: Review the schema, queries, and migration implications for this backend change.
